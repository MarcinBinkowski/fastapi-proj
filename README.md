...
- 🎯 Entry points to execute your program using `python -m <fastapi_proj>` or `$ fastapi_proj` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/MarcinBinkowski/fastapi-proj/branch/main/graph/badge.svg?token=fastapi-proj_token_here)](https://codecov.io/gh/MarcinBinkowski/fastapi-proj)
[![CI](https://github.com/MarcinBinkowski/fastapi-proj/actions/workflows/main.yml/badge.svg)](https://github.com/MarcinBinkowski/fastapi-proj/actions/workflows/main.yml)

Awesome fastapi_proj created by MarcinBinkowski

## Install

from source
```bash
git clone https://github.com/MarcinBinkowski/fastapi-proj fastapi_proj
cd fastapi_proj
make install
```
...
```bash
pip install fastapi_proj
```

## Executing

```bash
$ fastapi_proj run --port 8080
```

or

```bash
python -m fastapi_proj run --port 8080
```

or

```bash
$ uvicorn fastapi_proj:app
```
...
```bash
❯ fastapi_proj --help
Usage: fastapi_proj [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fastapi_proj create-user --help
Usage: fastapi_proj create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fastapi_proj create-user admin admin --superuser
```
...
Run with `fastapi_proj run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastapi_proj create-user`.
...
-----------------------------------------------------
fastapi_proj/__init__.py              4      0   100%
fastapi_proj/app.py                  16      1    94%
fastapi_proj/cli.py                  21      0   100%
fastapi_proj/config.py                5      0   100%
fastapi_proj/db.py                   10      0   100%
fastapi_proj/models/__init__.py       0      0   100%
fastapi_proj/models/content.py       47      1    98%
fastapi_proj/routes/__init__.py      11      0   100%
fastapi_proj/routes/content.py       52     25    52%
fastapi_proj/routes/security.py      15      1    93%
fastapi_proj/routes/user.py          52     26    50%
fastapi_proj/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastapi_proj/default.toml.
